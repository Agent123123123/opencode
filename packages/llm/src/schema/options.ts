...
export class HttpOptions extends Schema.Class<HttpOptions>("LLM.HttpOptions")({
  ...
  timeout: Schema.optional(Schema.Union([Schema.Finite.check(Schema.isGreaterThan(0)), Schema.Literal(false)])),
  headerTimeout: Schema.optional(Schema.Union([Schema.Finite.check(Schema.isGreaterThan(0)), Schema.Literal(false)])),
  chunkTimeout: Schema.optional(Schema.Finite.check(Schema.isGreaterThan(0))),
}) {}
...
export const mergeHttpOptions = (...items: ReadonlyArray<HttpOptions | undefined>): HttpOptions | undefined => {
  ...
  if (!body && !headers && !query && timeout === undefined && headerTimeout === undefined && chunkTimeout === undefined) return undefined
