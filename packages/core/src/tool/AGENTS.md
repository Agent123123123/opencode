...
The registry has no `PermissionV2.Service` dependency. Every settlement does use the Location-scoped `ToolExecutionPolicy.Service`, which wraps the decoded invocation around the leaf executor so semantic guards and lifecycle observers apply uniformly to built-in and plugin tools. An internal built-in-only operation attaches a permission action solely to preserve whole-tool definition filtering; it is not part of public `Tool.make`. Most tools default to their registered name; `edit`, `write`, and `apply_patch` declare the shared `edit` action.
