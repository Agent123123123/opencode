...
For providers whose ID starts with `opencode`, the runner supplies request-level HTTP identity through the existing LLM transport: `x-opencode-session` is the Session ID, `x-opencode-request` is the durable activity Turn ID, and `x-opencode-project` is the Location's Project ID. Transport retries and tool continuations retain that Turn ID. `x-opencode-client` follows the native client flag; `User-Agent` uses the actual executable product and release version supplied by its build. These identity headers override stale catalog values case-insensitively while preserving transport authentication and unrelated configured headers. Other providers receive no additional identity headers from this path.

Durable Session history and model-visible conversation context are distinct sets. Durable messages remain the complete transcript and execution record. Before every provider turn, Core derives a model-context projection from the active history window. Positive normal settlement is necessary but not sufficient for assistant narrative replay: `time.completed` and a non-empty, non-error `finish` must be present and `error` must be absent. A `finish=length` assistant that contains reasoning but has neither non-empty text nor a tool call is a settled physical generation without a replayable assistant utterance; its narrative is omitted before same-model metadata reuse or cross-model reasoning conversion. Failed, incomplete, and length-exhausted reasoning-only assistant narrative remains durable and transcript-visible but is omitted from later provider requests. Terminal local and provider-executed tool calls/results remain model-visible independently of the owning assistant's narrative outcome because they record effects that have already settled. After abandoned tools are reconciled, any remaining historical `pending` or `running` tool is a local projection invariant failure and no provider request is sent.
...
Compaction keeps the full transcript durable while replacing its active model representation with one hidden checkpoint containing a structured rolling summary and token-bounded serialized recent context. Its source is the same grouped model-context projection used by the provider request, never raw durable assistant content. Content excluded from normal model context therefore cannot re-enter through a summary or retained recent context, while a terminal tool call/result pair stays in one source group. Provider-native assistant, reasoning, and tool messages never survive across the boundary, avoiding signature and encrypted-reasoning failures when the earlier prefix changes.
